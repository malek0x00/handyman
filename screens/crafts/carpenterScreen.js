...
export default function Carpenter({ navigation }) {
  ...
  const [rows, setRows] = useState();
      ...
      async function loadData(){
        ...
        if (rows){
          rows.map(worker => {
            wsk.push(<MenuItem name={worker.name} uid={worker.uid} city={worker.city} phone={worker.phone} description={worker.description} avcolor="#62342b"/>)
          })
        }
  ...
  return (
    <NativeBaseProvider theme={theme}>
    <View style={styles.container}>
    <Text fontSize={50} style={{fontFamily: 'nexaHeavy', color: '#62342b',position:'absolute', top:70,left:20}}>Carpenters</Text>
