...
import { NativeBaseProvider, Text,VStack ,Spinner, ScrollView,extendTheme,Heading} from "native-base";
...
export default function Painter({ navigation }) {
    const [loaded] = useFonts({
     ...
     async function loadData(){
        ...
        if (rows){
          rows.map(worker => {
            wsk.push(<MenuItem name={worker.name} description={worker.description} uid={worker.uid} city={worker.city} phone={worker.phone} avcolor="#407076"/>)
          })
        }
